...
| `ethyca.com/guides/about-consent-management` | — | Investigated as a possible third example (user suggestion) — turned out to be a dev/demo configuration, not a representative production deployment: `Fides.options.fidesJsBaseUrl` points at `http://localhost:3000`, no banner ever renders, no footer settings trigger found, `tcfEnabled: false`. Not added to `tests/sites.json`; noted here only so a future session doesn't re-investigate it expecting a real third variant. |
