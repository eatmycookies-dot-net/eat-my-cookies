...
| `spiegel.de` | Needs implementation | Was listed as Automation-covered (Sourcepoint); moved here August 8, 2026. Still genuinely Sourcepoint (confirmed via live DOM inspection), but the first-layer banner is now a consent-or-pay wall (`Consent and continue` / `Subscribe now` / `Preferences`) with no direct Reject All control. Rejecting requires opening "Preferences" into a separate privacy-manager frame that uses per-category Accept/Reject button pairs instead of a bulk reject-all — `sp-frame-handler.js`'s `rejectFromPrivacyManager()` isn't built for that layout and currently does nothing (confirmed: zero `ACTION_FIRED` events reach the background script on this site). See `docs/cmp-impact-map.md`'s Sourcepoint section for the full finding and the required fix shape. |
...
| `Usercentrics` | Fixture-covered, pending live validation | User-supplied evidence from July 30, 2026 showed `leadersisland.com` using the modern Usercentrics shadow-root UI (`#usercentrics-cmp-ui`). Generic shadow handling remains covered, but Leaders Island is host-gated to the MAIN-world `UC_UI` path because DOM clicks can dismiss the modal without proving service consent persistence. The MAIN-world handler observes shadow-root mutations so a late-rendered modal is still handled after the document-level retry window, treats opacity-zero/pointer-events-disabled shells as dismissed for counting, and maps the publisher's analytics-described `Functional` category to the extension's Analytics setting. Extension-level regression coverage verifies stats for shadow DOM Accept All, Reject All, and Custom after 5-second delayed visibility, semantic category mapping, fade-out dismissal, and CMP-triggered reload, plus `window.UC_UI` API Accept, Reject, and Custom with persisted service decisions. Local headless probing did not surface a fresh banner, so this remains pending live/human recheck before claiming broad live coverage. |
