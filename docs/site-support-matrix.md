...
| `elpais.com` | Supported | Human-validated. The earlier "Accept, Reject, and Custom behaved correctly" note was wrong for Custom — Didomi's Tier 2 handler silently collapsed Custom into a full reject with no per-purpose logic (a generic, pre-existing bug, not elpais.com-specific). Fixed 2026-08-10/11: real per-purpose consent via `Didomi.setUserConsentStatusForAll`, plus two follow-on fixes the user caught live — a leftover PRISA `#acceptationCMPWall` backdrop ("shadow" over the dismissed page) and a `cmp-scroll-lock` class left on `<body>` after removing it. See `docs/cmp-impact-map.md` for the full writeup. |
...
| `nytimes.com` | Automation-covered | Was mislabeled Sourcepoint; a live probe on August 8, 2026 found the real CMP is **Fides**. Real support shipped August 9, 2026 across three same-day fixes: (1) the base implementation — notice-level `window.Fides.updateConsent()` for non-TCF/US sessions, real button clicks for TCF/GDPR sessions (this site's on-screen banner shows Reject All/Accept All/Manage Preferences directly); (2) a return-visit resync fix — Fides stops showing the banner once any decision is recorded, so changing the extension's own preference and revisiting previously did nothing until `Fides.showModal()` was added to force it back open; (3) a race-condition fix — the banner visually disappearing does not mean Fides has finished computing consent internally, so reopening "Privacy Choices" quickly after Accept/Reject could show stale/incomplete toggles until the fix started waiting for Fides' own `FidesUpdated` completion event. Live-verified via the real extension across the full accept/reject/custom × ccpaDoNotSell × VPN/non-VPN matrix (20 runs total across both sites), including the footer-reopen path checked immediately (not after a generous wait) on every VPN/TCF case. Custom in the TCF/EU path is currently treated as a full reject (honestly labeled `cmp_api:Fides:tcf_custom_as_reject`), matching the same granular-toggle limitation already documented for spiegel.de's Sourcepoint privacy manager. See `docs/cmp-impact-map.md`'s "Fides" section for the full writeup of all three fixes — none were caught by the original implementation's own testing; each was reported live by a user. |
| `wired.com` | Automation-covered | Same Fides migration and the same three same-day fixes as nytimes.com (base TCF support, return-visit resync, race-condition fix). This site's on-screen banner offers only Accept All + "Your Privacy Choices" — no direct Reject All until that's opened — unlike nytimes.com's banner, which shows all three options directly; the fix handles both shapes with one generic escalation path, not a per-site branch. Live-verified via the real extension across the same 20-combination matrix as nytimes.com. |
| `arstechnica.com` | Automation-covered | Was mislabeled Sourcepoint in `tests/sites.json`; user-confirmed live 2026-08-10 that the real CMP is **Fides**, same generic support that covers nytimes.com/wired.com — no site-specific work needed. Reclassified in `tests/sites.json` (`cmp: "Fides"`, `tier: "cmp-api"`) and verified via `node tests/validate.js --site="Ars Technica"`. |
